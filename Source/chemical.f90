...
!-------------------------------------------------------------------	
	subroutine read_elements(abundance)
	real(kind=8) :: abundance(:)
	integer :: i
		open(unit=42,file='DATA/ioniz_potential_chemeq.dat',status='old',action='read')
		read(42,*) 
		
		do i = 1, 21
			read(42,*) elements(i), pot_ion(i), afinidad(i)
! ! OLD			
! 			abund_atom(i) = 10.d0**(abund_atom(i)-12.d0)
			
! Use the element abundance read in the main program and enhanced as desired
...
!-------------------------------------------------------------------
	...
	real(kind=8) :: data(7)
		...
		open(unit=42,file='DATA/atomic_partition.dat',status='old',action='read')
...
!-----------------------------------------------------------------
	function calculate_abundance_Pe_from_T_Pg(mol_code, n_grid, abundance, height_in, temper_in, PT_in, &
	...
	real(kind=8) :: calculate_abundance_Pe_from_T_Pg(n_grid), abun_out(n_grid), abundance(:)
...
!  PH2plus_out : partial pressure of H2+ molecules (dyn/cm^2)
!-----------------------------------------------------------------
	function calculate_abundance_Pg_from_T_Pe(mol_code, n_grid, abundance, height_in, temper_in, Pe_in, &
		PH_out, PHminus_out, PHplus_out, PH2_out, PH2plus_out, P_total)
	integer :: n_grid, mol_code(:)
	real(kind=8) :: height_in(n_grid), temper_in(n_grid), Pe_in(n_grid), n_e_in(n_grid), abundance(:)
	real(kind=8) :: calculate_abundance_Pg_from_T_Pe(size(mol_code),n_grid), abun_out(size(mol_code),n_grid), initial_values(22)
	real(kind=8), dimension(n_grid) :: PH_out, PHminus_out, PHplus_out, PH2_out, PH2plus_out, P_total
	character(len=40) :: abundance_file
	real(kind=8) :: mole(273), height, minim_ioniz
	integer :: i, ind, l, loop	, minim, loop_mol, nmolec
...
!  PH2plus_out : partial pressure of H2+ molecules (dyn/cm^2)
!-----------------------------------------------------------------
	function calculate_abundance_from_T_Pe_Pg(mol_code, n_grid, abundance, height_in, temper_in, Pe_in, Pt_in,&
	...
	real(kind=8) :: calculate_abundance_from_T_Pe_Pg(size(mol_code),n_grid), abun_out(n_grid), initial_values(21),abundance(:)
	real(kind=8), dimension(n_grid) :: PH_out, PHminus_out, PHplus_out, PH2_out, PH2plus_out
	character(len=40) :: abundance_file
	real(kind=8) :: mole(273), height, minim_ioniz
	integer :: i, ind, l, loop	, minim, nmolec, loop_mol
...
! Initialize assuming that the total gas pressure is given by H
			...
			enddo
	 		...
	 		do loop_mol = 1, nmolec
				...
				if (.not.(mole(i)>0) .and. .not.(mole(i)<=0) ) mole(i) = 0.d0
! 				write(45,*) height, mole(i)
				calculate_abundance_from_T_Pe_Pg(loop_mol,loop) = mole(i)
			enddo
